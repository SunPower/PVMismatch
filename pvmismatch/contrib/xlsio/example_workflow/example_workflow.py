...
pv_sys = pvsystem.PVsystem(numberStrs=str_num, pvstrs=[pv_str]*str_num,
                           numberMods=[str_len]*str_num, pvmods=[pv_mod]*str_len)
...
system_layout_to_xls(output_xls_name, pv_sys, write_bpd_act=False)
...
# After a human has changed the irradiance values for the system in the xls and saved it with a different name, we can
...
for i in list(range(1,7)):
    ...
    system_layout_to_xls(output_xls_name=input_xls_name, pv_sys=pv_sys, write_bpd_act=True) # updating the input excel with the bypass diode activation
    pv_sys.plotSys()
    plt.show(block=False)
