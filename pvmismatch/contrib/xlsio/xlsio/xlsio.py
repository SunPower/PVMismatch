...
def _create_nan_df(pv_mod, nr_string, nr_mod):
    ...
    cell_pos = pv_mod.cell_pos
    nrows = int(pv_mod.numberCells / sum(pv_mod.subStrCells))
    nan_df = pd.DataFrame(index=['{}_{}'.format(nr_mod, nr) for nr in range(nrows)])
    for b, bypass in enumerate(cell_pos):
        for c, col in enumerate(bypass):
...
def system_layout_to_xls(output_xls_name, pv_sys, write_bpd_act):
    ...
    If "write_bpd_act" is True, bypass diode activation is checked and on the ActiveBpd tab bypassed cells
    are represented with 0 and non-bypassed cells with 1."""
    ...
    writer.sheets['ActiveBpd'] = workbook.add_worksheet('ActiveBpd')
    if write_bpd_act:
    ...
    for s, string in enumerate(pv_sys.pvstrs):
        ...
        for m, module in enumerate(string.pvmods):
            cell_pos_df = _create_cell_pos_df(pv_mod=module, nr_string=s, nr_mod=m)
            ...
            if write_bpd_act:
                ...
                for ss in range(module.numSubStr):
                    interp_substring_vi = interp1d(module.Isubstr[ss], module.Vsubstr[ss])
                    ...
                    if substring_vmp < 0:   # doublecheck if we should compare to 0 here
                        ...
                        [bpd.append(1) for nss in range(cols_per_substr[ss])]
                bpd_df = pd.DataFrame(index=cell_pos_df.index, columns=cell_pos_df.columns)
            ...
            if not write_bpd_act:
                bpd_df = _create_nan_df(pv_mod=module, nr_string=s, nr_mod=m)
            if s == 0:
                startcol = 0
            else:
                startcol = s*(ncols+1)
            ...
            bpd_df.to_excel(writer, sheet_name='ActiveBpd', startrow=startrow , startcol=startcol) # write irrad
    # formatting the Irradiance worksheet
    writer.sheets['Irradiance'].conditional_format(0, 0, writer.sheets['Irradiance'].dim_rowmax,
    ...
    writer.sheets['CellTemp'].conditional_format(0, 0, writer.sheets['CellTemp'].dim_rowmax,
                                                   ...
                                                   {'type': '3_color_scale',
                                                    ...
                                                    'mid_color':'#E5E7E9',
                                                    'max_color':'#E74C3C'})
    # formatting bpd ActiveBpd worksheet
    writer.sheets['ActiveBpd'].conditional_format(0, 0, writer.sheets['ActiveBpd'].dim_rowmax,
                                                   writer.sheets['Irradiance'].dim_colmax,
                                                   {'type': '2_color_scale',
                                                    'min_type': 'num',
                                                    'max_type': 'num',
    ...
    for string in list(range(str_num)):
        for module in list(range(str_len)):
            ...
            irrad = pd.read_excel(input_xls_name, sheet_name='Irradiance',
                                  ...
                                  usecols=range(string*(ncols+1), (string+1)*(ncols+1)),
