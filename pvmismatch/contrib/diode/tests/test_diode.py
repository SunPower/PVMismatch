...
def test_fid():
    ...
    # evaluate arrays
    test_data = (np.array([ISAT1_2,ISAT2_2]), np.array([V_D, V_D]),
    ...
    fid_expected = f__i_d(*test_data)
    jid_expected = np.array([
        f__d__i_sat(*test_data), f__d__v_d(*test_data),
        f__d__m(*test_data), f__d__v_t(*test_data)
    ], dtype=np.float)
...
def test_fvd():
    ...
    # evaluate
    i_d, i_sh =  0.00190355, 0.01771217
    test_data = {'v_c': V_D - RS_1 * (ISC0 - i_d - i_sh),
                 'i_c': ISC0 - i_d - i_sh, 'r_s': RS_1}
